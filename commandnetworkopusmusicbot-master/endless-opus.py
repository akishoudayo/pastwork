...
os.environ['VCID'] = '792571486157406208'
os.environ['QUEUEID'] = '792346258801950740'
...
def finalize(info_dict):
	try:
		if os.path.isfile('{}.m4a'.format(info_dict['id'])):
			convert = subprocess.run("mobile-ffmpeg-audio-4.4.aar -i {0}.m4a -vn -b:a 192000 -c:a libopus -n {0}.opus".format(info_dict['id']), shell=True)
			data = json.loads(subprocess.run("mobile-ffmpeg-audio-4.4 -i {}.opus -print_format json -show_streams  -show_format -loglevel quiet".format(info_dict['id']), stdout=subprocess.PIPE, shell=True).stdout)
			info_dict['format'] = data['format']
			info_dict['streams'] = data['streams']
			info_dict['path'] = info_dict['id'] + '.opus'
			return info_dict
		else:
			data = json.loads(subprocess.run("mobile-ffmpeg-audio-4.4.aar -i {}.opus -print_format json -show_streams  -show_format -loglevel quiet".format(info_dict['id']), stdout=subprocess.PIPE, shell=True).stdout)
			info_dict['format'] = data['format']
			info_dict['streams'] = data['streams']
			info_dict['path'] = info_dict['id'] + '.opus'
			return info_dict
